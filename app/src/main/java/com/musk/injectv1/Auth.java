...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    public String doInBackground(String... strings) {
        ...
        try {
            ...
            data.put("app_Us", strings[0]);
            data.put("app_Pa", strings[1]);
            data.put("app_Lo", "1");
            data.put("app_Id", getUniqueId(getActivity()));
            token.put("Data", encrypt(data.toString(), this.puk));
            ...
            HttpsURLConnection urlConnection = (HttpsURLConnection) new URL(Logg()).openConnection();
    ...
    public void onPostExecute(String s) {
        ...
        if (activity != null) {
            ...
            if (s == null || s.isEmpty()) {
                Toast.makeText(activity, "404 : Erro desconhecido", 1).show();
                ...
                try {
                    ...
                    if (!verify(decData, ack.get("Sign").toString(), this.puk)) {
                        Toast.makeText(activity, "Token incorreto!", 1).show();
                        return;
                    }
                    JSONObject data = new JSONObject(decData);
                    if (data.get("Status").toString().equals("Success")) {
                           Toast.makeText(activity, "Login Successfully!", 0).show();
                    ...
                    Toast.makeText(activity, data.get("MessageString").toString(), 1).show();
                } catch (Exception e) {
                    e.printStackTrace();
                    Toast.makeText(activity, "Invalid Result", 1).show();
                }
            }
        }
    }
    ...
    public final MainActivity getActivity() {
        return (MainActivity) this.weakActivity.get();
    }
    ...
    public final PublicKey getPublicKey(byte[] keyBytes) throws Exception {
        return KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(keyBytes));
    }

    public final String encrypt(String plainText, byte[] keyBytes) throws Exception {
        Cipher encryptCipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
        encryptCipher.init(1, getPublicKey(keyBytes));
        return Utils.toBase64(encryptCipher.doFinal(plainText.getBytes(StandardCharsets.UTF_8)));
    }

    public final boolean verify(String plainText, String signature, byte[] keyBytes) throws Exception {
