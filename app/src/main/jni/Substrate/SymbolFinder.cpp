...
#define TAG OBFUSCATE("MSHook")
#define LOGD(msg) __android_log_print(ANDROID_LOG_DEBUG, TAG, "%s", (const char*)(msg))
#define LOGE(msg) __android_log_print(ANDROID_LOG_ERROR, TAG, "%s", (const char*)(msg))

#define MAX_NAME_LEN 256
static const char* const MEMORY_ONLY = "[memory]";
...
static void *xmalloc(size_t size) {
    ...
    p = malloc(size);
    if (!p) {
        const char* msg = OBFUSCATE("Out of memory\n");
        printf("%s", msg);
        exit(1);
    }
    return p;
...
static int load_memmap(pid_t pid, struct mm *mm, int *nmmp) {
    static const char* PROC_MAP_FORMAT = OBFUSCATE("/proc/%d/maps");
    ...
    sprintf(p_buf, PROC_MAP_FORMAT, pid);
    fd = open(p_buf, O_RDONLY);
    if (0 > fd) {
    const char* msg = OBFUSCATE("Can't open %s for reading\n");
    LOGE(msg);
    free(p_buf);
    return -1;
}
    ...
    m = mm;
    while (p) {
        static const char* MAP_FORMAT = OBFUSCATE("%08lx-%08lx %*s %*s %*s %*s %s\n");
        rv = sscanf(p, MAP_FORMAT, &start, &end, name);
        ...
        if (rv == 2) {
            ...
            m->start = start;
            m->end = end;
            memcpy(m->name, MEMORY_ONLY, strlen(MEMORY_ONLY) + 1);
            continue;
        }
...
static int find_libname(const char *libn, char *name, int len, unsigned long *start,
    ...
    for (i = 0, m = mm; i < nmm; i++, m++) {
        ...
        static const char* SO = OBFUSCATE("so");
        if (!strncmp(SO, p, 2) || 1)
            break;
    }
    if (i >= nmm)
...
int find_name(pid_t pid, const char *name, const char *libn,
    ...
    const char* msg = OBFUSCATE("cannot read memory map\n");
    LOGD(msg);
    return -1;
}
    ...
    if (0 > lookup_func_sym(s, (char *) name, addr)) {
        LOGD(OBFUSCATE("cannot find function\n"));
        return -1;
    }
    *addr += libcaddr;
    return 0;
